...
BA.GameMap = class GameMap {
    constructor(mapType = 'farm', seed = 0.5) {
    ...
    _generateFarm() {
        ...
        // === BUILDINGS ===
        // Central fortress
        this._buildRoom(55, 40, 14, 12);
        this._buildWall(62, 41, 62, 50); // vertical divider
        this.data[44][62] = BA.TILE.DOOR;
        this.data[45][62] = BA.TILE.DOOR;

        // More rooms spread out
        this._buildRoom(10, 10, 10, 8);
        this._buildRoom(95, 10, 12, 9);
        this._buildRoom(12, 70, 10, 8);
        this._buildRoom(92, 68, 14, 10);

        // Sub buildings
        this._buildRoom(30, 20, 8, 6);
        this._buildRoom(78, 22, 8, 6);
        this._buildRoom(32, 60, 9, 7);
        this._buildRoom(75, 58, 8, 6);

        // Mid perimeter shelters
        this._buildRoom(4, 42, 6, 6);
        this._buildRoom(110, 42, 6, 6);

        // === TREE CLUSTERS (Forests to hide in) ===
        // Large forest zones
        this._placeCluster(BA.TILE.TREE, 25, 4, 18, 12, 0.45);
        this._placeCluster(BA.TILE.TREE, 75, 4, 16, 12, 0.45);
        this._placeCluster(BA.TILE.TREE, 4, 52, 14, 14, 0.4);
        this._placeCluster(BA.TILE.TREE, 102, 52, 14, 14, 0.4);
        this._placeCluster(BA.TILE.TREE, 48, 22, 22, 10, 0.35); // center-top
        this._placeCluster(BA.TILE.TREE, 48, 62, 22, 10, 0.35); // center-bottom

        // Scattered clusters
        this._placeCluster(BA.TILE.TREE, 20, 35, 10, 8, 0.3);
        this._placeCluster(BA.TILE.TREE, 88, 35, 10, 8, 0.3);
        this._placeCluster(BA.TILE.TREE, 55, 78, 12, 8, 0.3);

        // Single tree scatter
        this._scatterTiles(BA.TILE.TREE, 120, 2, this.cols - 4, 2, this.rows - 4);

        // === ROCKS ===
        this._placeCluster(BA.TILE.ROCK, 40, 12, 6, 4, 0.45);
        this._placeCluster(BA.TILE.ROCK, 74, 12, 6, 4, 0.45);
        this._placeCluster(BA.TILE.ROCK, 20, 62, 5, 5, 0.4);
        this._placeCluster(BA.TILE.ROCK, 92, 62, 5, 5, 0.4);
        this._placeCluster(BA.TILE.ROCK, 58, 34, 4, 3, 0.5);
        this._placeCluster(BA.TILE.ROCK, 58, 55, 4, 3, 0.5);
        this._scatterTiles(BA.TILE.ROCK, 60, 2, this.cols - 4, 2, this.rows - 4);

        // === WATER ===
        // Giant lake/ponds
        this._placeOval(BA.TILE.WATER, 20, 26, 6, 4);
        this._placeOval(BA.TILE.WATER, 100, 26, 6, 4);
        this._placeOval(BA.TILE.WATER, 60, 10, 7, 3);
        this._placeOval(BA.TILE.WATER, 60, 78, 7, 3);
    ...
    _generateCave() {
        ...
        this._scatterTiles(BA.TILE.TREE, 160, 2, this.cols - 4, 2, this.rows - 4);
    ...
    _generateVillage() {
        ...
        // Center square
        this._placeOval(BA.TILE.SAND, Math.floor(this.cols / 2), Math.floor(this.rows / 2), 12, 9);
        ...
        this._placeCluster(BA.TILE.TREE, 4, 4, 15, 14, 0.4);
        this._placeCluster(BA.TILE.TREE, this.cols - 20, 4, 15, 14, 0.4);
        this._placeCluster(BA.TILE.TREE, 4, this.rows - 20, 15, 14, 0.4);
        this._placeCluster(BA.TILE.TREE, this.cols - 20, this.rows - 20, 15, 14, 0.4);
        this._scatterTiles(BA.TILE.TREE, 180, 2, this.cols - 4, 2, this.rows - 4);
    ...
    _generateCity() {
        ...
        // Blocks columns & rows coordinates
        const colBlocks = [5, 32, 60, 88, 115];
        const rowBlocks = [5, 26, 48, 70, 88];
        ...
        this._placeOval(BA.TILE.SAND, cx, cy, 14, 10);
        this._placeCluster(BA.TILE.TREE, cx - 8, cy - 6, 16, 12, 0.55);

        // Concrete barriers/street obstacles (reusing ROCK)
        this._scatterTiles(BA.TILE.ROCK, 85, 2, this.cols - 4, 2, this.rows - 4);
        
        // Puddles
        this._placeOval(BA.TILE.WATER, 15, 45, 6, 3);
        this._placeOval(BA.TILE.WATER, 105, 45, 6, 3);
    ...
    _generateSpawns() {
        ...
        const spawnCandidates = [
            ...
            { x: 20, y: 70 }, { x: 100, y: 70 },
            { x: 40, y: 30 }, { x: 80, y: 30 },
            { x: 40, y: 60 }, { x: 80, y: 60 },
            // Outer mids
            { x: 10, y: 45 }, { x: 110, y: 45 },
            { x: 60, y: 18 }, { x: 60, y: 72 },
            // Center ring
            { x: 50, y: 45 }, { x: 70, y: 45 },
            { x: 60, y: 36 }, { x: 60, y: 54 },
